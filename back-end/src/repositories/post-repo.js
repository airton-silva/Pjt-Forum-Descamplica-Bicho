...
exports.save = async (post) => {
  const result = await pool.query(
    "INSERT INTO posts(title, body, created_at, updated_at, user_id, resumo) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *;",
    [post.title, post.body, post.created_at, post.updated_at, post.user_id, post.resumo]
  );
...
exports.findAll = async () => {
  const result = await pool.query("SELECT users.id, users.name, users.email, posts.created_at,"
                                  + " posts.id, posts.updated_at, posts.body, posts.resumo "+
...
exports.update = async (id, post) => {
  const result = await pool.query(
    "UPDATE posts SET title=$1, body=$2, created_at=$3, updated_at=$4, resumo=$5 WHERE id=$6 RETURNING *;",
